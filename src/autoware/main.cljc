(ns autoware.main
  "Kotodama WASM entrypoint for the Autoware clean-room actor (L4) — Clojure port.
  ...
  facts (`autoware.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "autoware")
(def tier "L4")
...
(def entity-specs
  [{:entity "OperationModeState" :plural "operationmodestates" :id-prefix "autoware_ope"
    :fields [:stamp :mode :isAutowareControlEnabled :isInTransition :isStopModeAvailable :isAutonomousModeAvailable :isLocalModeAvailable :isRemoteModeAvailable]
    :required [:stamp :mode]
    :coerce {:mode :int :isAutowareControlEnabled :bool :isInTransition :bool :isStopModeAvailable :bool :isAutonomousModeAvailable :bool :isLocalModeAvailable :bool :isRemoteModeAvailable :bool}
    :refs {}}
   {:entity "RouteState" :plural "routestates" :id-prefix "autoware_rou"
    :fields [:stamp :state]
    :required [:stamp :state]
    :coerce {:state :int}
    :refs {}}
   {:entity "LocalizationInitializationState" :plural "localizationinitializationstates" :id-prefix "autoware_loc"
    :fields [:stamp :state]
    :required [:stamp :state]
    :coerce {:state :int}
    :refs {}}
   {:entity "MotionState" :plural "motionstates" :id-prefix "autoware_mot"
    :fields [:stamp :state]
    :required [:stamp :state]
    :coerce {:state :int}
    :refs {}}
   {:entity "Gear" :plural "gears" :id-prefix "autoware_gea"
    :fields [:status]
    :required [:status]
    :coerce {:status :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"autoware.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "autoware-compat" :tier tier :entities entities} 200])
...
(defn start! [] :autoware-compat/ready)
